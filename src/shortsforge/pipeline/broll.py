...
async def insert_broll(
    ...
    src = safe_resolve(clip_path, allowed_roots=[
        Path("output").resolve(), Path("samples").resolve()
    ])
    ...
    filter_parts = []
    for i, t in enumerate(insert_times):
        filter_parts.append(
    ...
    if filter_parts:
        ...
        subprocess.run(  # noqa: S603
            ...
            shell=False, stdin=subprocess.DEVNULL, check=True, capture_output=True,
        )
    else:
        import shutil
        shutil.copy2(str(src), str(dst))
